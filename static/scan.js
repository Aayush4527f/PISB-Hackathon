document.addEventListener("DOMContentLoaded", async () => {
  ...
  let role_data = await response.json();
  let role = role_data.role;
  console.log(role);
  ...
  function populateDoctors(doctors) {
    ...
    // Show section if there are doctors
    if (doctors.length > 0) {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.diagnosis && role === "patient") {
          const response = await fetch("/api/user/get_doctors", { method: "POST" });

          const res_data = await response.json();
          if(!res_data.success) console.log("An error occured while getting doctors list");
          else populateDoctors(res_data.doctors);
        }
      } else {
