...
function renderTable(data) {
  ...
  data.forEach((item) => {
    ...
    row.querySelectorAll(".view-btn")[0].addEventListener("click", () => showNote(item.note));
    row.querySelectorAll(".view-btn")[1].addEventListener("click", () => showSummary(item.summary));
    row.querySelector(".delete-btn").addEventListener("click", () => deleteScan(item.patient_id));
...
function showSummary(summary) {
  summaryText.innerHTML = formatGeminiOutput(summary || "No summary available.");
